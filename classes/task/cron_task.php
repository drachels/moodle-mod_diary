...
public function execute() {
    ...
    if (!$entries) {
        //$this->log("No pending entries to mail.");
        $this->log("No pending entries to mail. Current time is ".date("l, F j, Y H:i:s", $timenow)." and the edittimeago is: ".date("l, F j, Y H:i:s", $edittimeago).".");
    ...
    foreach ($entries as $entry) {
        try {
            ...
            foreach ($teachers as $teacher) {
                ...
                //$this->log("Line 110 - Debugging entry fields: " . implode(', ', array_keys((array)$entry)));
                ...
                if (empty($entry->submissionemail)) {
                    ...
                    //break; // No need to check other teachers if the whole diary is disabled
                ...
                if (!empty($entry->teacheremail)) {
                    ...
                    //break; // No need to check other teachers if the all teachers are disabled
                ...
                $message->subject           = fullname($student) . " submitted a diary entry.";
                $message->fullmessage       = "Hi,\n\n" . fullname($student) . " has submitted a new entry. \nView: {$diaryinfo->url}";
                $message->fullmessageformat = FORMAT_MARKDOWN;
                $message->fullmessagehtml   = "<p>Hi {$teacher->firstname},</p><p>" . fullname($student) . " submitted an entry.</p>";
