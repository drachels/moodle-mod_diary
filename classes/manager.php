...
class manager {
    ...
    /**
     * Check if the current user has added an entry in the diary.
     *
     * @return bool true if the user has answered, false otherwise
     */
    public function has_answered(): bool {
        global $USER;
        $answer = \diary_get_completion_state($this->course, $this->cm, $USER->id, true);
        return $answer;
    }

    /**
     * Return the count of users who can submit entries to this diary module, that the current user can see.
     *
    ...
    public function count_all_users(
        ...
        if (!has_capability('mod/diary:addentries', $this->context)) {
            return 0;
        }

        // Get all users with the capability to choose in this context.
        $users = get_users_by_capability($this->context, 'mod/diary:addentries', 'u.id');
    ...
    /**
     * Return the current count of users who have submitted entries to this diary module, that the current user can see.
     *
     * @param int[] $groupids the group identifiers to filter by, empty array means no filtering
     * @return int the number of entries that the user can see
     */
    public function count_all_users_answered(
        array $groupids = []
    ): int {
        if (!has_capability('mod/diary:manageentries', $this->context)) {
            return 0;
        }
        // Check to see if groups are being used in this diary.
        ...
        $entrycount = results::diary_count_entries($this->instance, $groupids);
