...
$data->id = $cm->id;
    ...
    if ($form->is_cancelled()) {
        ...
        if (!empty($fromform->entryid)) {
            $originalpromptid = $DB->get_field('diary_entries', 'promptid', ['id' => $fromform->entryid], MUST_EXIST);
            if ($fromform->promptid != $originalpromptid) {
        ...
        if ($fromform->entryid) {
            ...
            if (($entry) && (!($entry->timecreated == $newentry->timecreated))) {
                ...
                $newentry->entrycomment .= get_string(
                    'invalidtimechangeoriginal',
                    'diary',
                    [
                        'one' => userdate($entry->timecreated),
                    ]
                );
                $newentry->entrycomment .= get_string(
                    'invalidtimechangenewtime',
                    'diary',
                    [
                        'one' => userdate($newentry->timecreated),
                    ]
                );
        ...
        if ($data->text !== $newentry->text) {
            ...
            if (
                (get_config('mod_diary', 'teacheremail')) && ($diary->teacheremail || $diary->studentemail)
                ...
                foreach ($teachers as $teacher) {
                    ...
                    if (get_user_preferences('diary_emailpreference_' . $diary->id, null, $teacher->id) == 1) {
                        ...
                        $message->subject = fullname($USER) .
                            " has posted a diary entry in course '$course->shortname' using the edit.php file.";
